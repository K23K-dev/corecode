...
import { MAX_CODE_BYTES, RequestError } from './validation.mjs';

// Keep Node's file resolver: Next rewrites direct require.resolve calls to bundle module IDs.
const resolveRuntimeFile = Reflect.get(createRequire(import.meta.url), 'resolve');
...
function checkedPreview(problem) {
...
function frameRuntime(problem, preview) {
  return `(() => {
...
async function compileExample(problem, preview) {
      ...
      plugins: [
        {
          ...
          setup(bundle) {
            ...
            bundle.onLoad({ filter: /.*/, namespace: 'preview' }, (args) => ({
              ...
              loader: args.path === 'preview:react' ? 'js' : problem.extension,
            }));
          },
        },
      ],
...
export async function createPreviewDocument(problem) {

...
vi.mock('pg', () => ({
  Pool: class {
    constructor() {
      throw new Error('Offline streak tests must not open a database.');
    }
  },
}));
const repositoryModule = '../server/repository.mjs';
const { readActivity, repairActivity, initializeSchema } = await import(repositoryModule);
...
describe('fixed Eastern practice activity snapshots', () => {
  ...
  it('counts repeated accepted submissions but ignores invalid and future archive timestamps', async () => {
    ...
    expect((await readActivity(source.pool, NOW)).days).toEqual([{ date: '2024-03-01', count: 2 }]);
  });
